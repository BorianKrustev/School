	...
	public abstract class DbContext
	{
			...
			where TDbSet : class, new() where TCollection : class, new()
		{
			...
			if (isManyToMany)
			{
				...
				{
					var primaryKeyValu = foreignKey.GetValue(entity);

					var navigationEntities = navigationDbSet
						.Where(navigationEntity => primaryKey.GetValue(navigationEntity).Equals(primaryKeyValu))
						.ToArray();
			...
			where TEntity : class, new()
		{
			...
			{
				var navigationPropartyName = foreignKey.GetCustomAttribute<ForeignKeyAttribute>().Name;
				var navigationProperty = entityType.GetProperty(navigationPropartyName);

				var navigationDbSet = this.dbSetProperties[navigationProperty.PropertyType]
					.GetValue(this);

				var navigationPrymeryKey = navigationProperty.PropertyType.GetProperties()
					.First(pi => pi.HasAttribute<KeyAttribute>());

				foreach (var entity in dbSet)
				{
					var foreignKeyValu = foreignKey.GetValue(entity);

					var navigationPropartyValue = ((IEnumerable<object>) navigationDbSet)
						.First(curenNavigationProparty => navigationPrymeryKey.GetValue(curenNavigationProparty).Equals(foreignKeyValu));

					navigationProperty.SetValue(entity, navigationPropartyValue);
				}
			}
		}
			...
			where TEntity : class
		{
			...
			var fetcheRows = this.connection.FetchResultSet<TEntity>(tableName, columns).ToArray();
