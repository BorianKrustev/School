...
namespace _05_Hands_of_Cards
{
    class Program
    {
        static void Main()
        {
            ...
            while (input != "JOKER")
            {
                ...
                if (!people.ContainsKey(name))
                {
                    ...
                    AddCardsToPeople(people[name], cardArgs);
                }
                else
                {
                    AddCardsToPeople(people[name], cardArgs);
        ...
        private static void AddCardsToPeople(Dictionary<string, int> person, string[] cardArgs)
        {
            foreach (var card in cardArgs)
            {
                if (!person.ContainsKey(card))
                {
                    person.Add(card, GetCardValye(card));
                }
            }
            ;
        }

        private static int GetCardValye(string card)
        {
            int power = 0;
            switch (card[0])
            {
